...
def insert_data_to_postgresql(ti, **kwargs):
    ...
    try:
        ...
        values_for_hk_service_category = []
        values_for_hk_service = []
        values_for_hk_service_item = []
        ...
        for dynamo_item in data:
            ...
            hk_id = dynamo_item.get('id', {}).get('S', '')
            ...
            if hotel_id not in hotel_ids:
                ...
                store_skip_data_to_file(dynamo_item, 'No hotel in hotel table', 'skipped_hk.json')
                continue
            ...
            for item in items:
                ...
                if item_id == '' or str.isspace(item_id) or item_id is None:
                    logging.info(f"item id is invalid {item_id}")
                    continue
                    
                values_for_hk_service_item.append((
                   item_id,
                   item_name,
                   hk_id
                ))
                
            values_for_hk_service.append((
                hk_id,
                code,
                name,
                'HOUSEKEEPING_'+hotel_id
            ))
            
            values_for_hk_service_category.append((
                'HOUSEKEEPING_'+hotel_id,
                'HOUSEKEEPING',
                'housekeeping',
                hotel_id
        ...
        if values_for_hk_service_category:
            ...
            """
            extras.execute_values(cursor, insert_query_hk_service_category_table, values_for_hk_service_category)
            
            
            
        if values_for_hk_service:
            insert_query_hk_service_table = """
                ...
                VALUES %s ON CONFLICT(id) DO NOTHING;
            """
            extras.execute_values(cursor, insert_query_hk_service_table, values_for_hk_service)
            
        if values_for_hk_service_item:
            insert_query_hk_service_item_table = """
                ...
                VALUES %s ON CONFLICT(id) DO NOTHING;
            """
            extras.execute_values(cursor, insert_query_hk_service_item_table, values_for_hk_service_item)
            
        conn.commit()
        logging.info(f"Skipped {skip_count_no_hotel_id} records due to missing hotel ID.")
        logging.info(f"Inserted {len(values_for_hk_service)} values_for_hk_service records into PostgreSQL.")
        logging.info(f"Inserted {len(values_for_hk_service_category)} values_for_hk_service_category records into PostgreSQL.")
        logging.info(f"Inserted {len(values_for_hk_service_item)} values_for_hk_service_item records into PostgreSQL.") 
